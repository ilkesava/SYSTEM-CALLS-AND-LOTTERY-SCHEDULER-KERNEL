...
asmlinkage void schedule(void)
{
	...
	struct task_struct *prev, *next, *p , *xa;
...
repeat_schedule:
	...
	unsigned long sleeep_time = (jiffies-(p->sleep_time))/HZ;
	...
	if(sleeep_time <= 40){
	 ...
	 totalTicket+= p->ticket;
	}//if(canschedule)bitis
}//list for each bitis	

		//RANDOM NUMBER GENERATE
		get_random_bytes(&random_number,sizeof(unsigned long));
		random_number=(random_number%totalTicket)+1;
		x=(int)random_number;	
		...
		{
			p->flag = 1;
			int currentuid = p->uid;
			list_for_each(tmp, &runqueue_head) {
			xa = list_entry(tmp, struct task_struct, run_list);
			if (can_schedule(xa, this_cpu)) {
				if (currentuid == xa->uid)
				{
					xa->flag = 1;
				}
				}
